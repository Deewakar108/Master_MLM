...
{
    public class bm_pariwar_closing
    {
        ...
        public bool StartClosing(string StartDate, string EndDate, string ClosingNumber, string ClosingDate, string Closing_iDate, string DeleteID)
        {
            ...
            try
            {
                ...
                DataRow[] drMembers = dtAllMember.Select("Verification_iDate>=" + Start_iDate + " and  Verification_iDate<=" + End_iDate);
                if (drMembers.Length != 0)
                {
                    ...
                    while (dtStartDate <= dtEndDate)
                    {
                        ...
                        foreach (DataRow dr in drTempMember)
                        {
                            ...
                            string JoiningTime = dr["Joiningtime"].ToString();
                            JoiningAmount = int.Parse(dr["Joining_amount"].ToString());

                            UpdatePairIn_DailyChildTable(MemberCode, tempStartDate, ClosingNumber, DeleteID);
                        }
                    ...
                    if (IsClosingInProcess())         //Not Exist Then Entry
                    {
                        ...
                        if (!IsClosingExist(Start_iDate.ToString(), End_iDate.ToString()))
                        {
                            AddIn_Closin_Table(StartDate, EndDate, ClosingDate, ClosingNumber, DeleteID);
        ...
        public void UpdateReward(string ClosingNumber)
        {
            ...
            sql = "select * from Reward_list order by ID ASC";
            ...
            if (dtDistinctMember.Rows.Count != 0)
            {
                foreach (DataRow drMember in dtDistinctMember.Rows)
                {
                    ...
                    foreach (DataRow dr in dtReward.Rows)
                    {
                        ...
                        string RewardName = dr["Reward"].ToString();
                        ...
                        if (drPair.Length != 0)
                        {
                            if (!IsAchievedBefore(MemberCode, Level))
                            {
                                ...
                                sql = "insert into AchievedTable(MemberCode,DateOfAchievement,Status,RewardName,Level,idate,ClosingNumber) values ('" + MemberCode +
                                      "','" + DateOfAchievement + "','" + Status + "','" + RewardName + "','" + Level + "','" + idate + "','" + ClosingNumber + "')";
        ...
        public void UpdateDailyChildTable_Using_Business_Plan(string ClosingType, string ClosingNumber, string StartDate, string EndDate, string ClosingDate, string DeleteID)
        {
            ...
            if (dtMemberPair.Rows.Count != 0)
            {
                ...
                {
                    string MemberCode = drMember["MemberCode"].ToString();
                    if (MemberCode == "BMHP68424131")
                    {
                        string ss = "";
                    }
                    string MatchingIncome = GetMatchingIncome(MemberCode);
        ...
        public void UpdatePairIn_DailyChildTable(string MemberCode, string StartDate, string ClosingNumber, string DeleteID, bool IsIntervalWise = false, int NoOfInterval = 1)
        {
            ...
            else
            {
                ...
                if (drDailyChild.Length != 0)
                {
                    ...
                    //Update Pair Here.
                    PairInfo pairUpdate = GetUpdatePairValue(Left, Right, Pair, Position, SponsorCode, ClosingNumber, StartDate);
                    ...
                    UpdateInToDailyChildTableInDataBase(SponsorCode, StartDate, ClosingNumber, LeftChild, RightChild, PairChild);
                }
                else
                {
                    ...
                    InsertInToDailyChildTableInDataBase(ClosingNumber, DeleteID, Idate.ToString(), Date, Time, Membercode, Total_leftchild, Total_rightchild, NewPair);
                ...
                UpdatePairIn_DailyChildTable(SponsorCode, StartDate, ClosingNumber, DeleteID);
            }
        }

        public void AddPairInMemberPairLevel(int PairAchieved, string MemberCode, string AchievedDate, string ClosingNumber)
        {
            ...
            InsertInToMemberPairLevelInDataBase(MemberCode, ClosingNumber, PairAchieved.ToString(), AchievedDate);
        }
        ...
        public PairInfo GetUpdatePairValue(int Left, int Right, int Pair, string Position, string MemberCode, string ClosingNumber, string AchievedDate)
        {
            PairInfo p = new PairInfo();

            if (Position == "LEFT") { Left = Left + 1; }
            if (Position == "RIGHT") { Right = Right + 1; }
            ...
            if (iPair.IsPairValid)
            {
                if (Left >= iPair.Left && Right >= iPair.Right)
                {
                    ...
                    //AddPairInMemberPairLevel(PrevPair + 1, MemberCode, AchievedDate, ClosingNumber);
                    InsertInToMemberPairLevelInDataBase(MemberCode, ClosingNumber, (PrevPair + 1).ToString(), AchievedDate);
                }
            }
        ...
        {
            string sql = "select * from closing where  I_Start_date='" + Start_iDate + "' and I_End_date='" + End_iDate + "'";
            DataTable dtTemp = imp.FillTable(sql);
        ...
        public void InsertInToMemberPairLevelInDataBase(string MemberCode, string ClosingNumber, string PairAchieved, string AchievedDate)
        {
            string IsUsed = "0";
        ...
        public void AddIn_Closin_Table(string StartDate, string EndDate, string ClosingDate, string closingNumber, string Deleteid)
        {
            string Idate = imp.ConvertStringTo_iDate(ClosingDate);
            ...
            string sql = "insert into closing(Start_date, End_date, Date, Closingno, Idate, Deleteid, Closing_Time, I_Start_date, I_End_date, ClosingStatus) values ('" +
                         StartDate + "','" + EndDate + "','" + ClosingDate + "','" + closingNumber + "','" + Idate + "','" + Deleteid + "','" + Closing_Time + "','" +
                         I_Start_date + "','" + I_End_date + "','InProcess')";
            int i = imp.InsertUpdateDelete(sql);
        }
        ...
        public void InsertInToDailyChildTableInDataBase(string ClosingNumber, string DeleteID, string Idate, string Date, string Time,
                                                        string MemberCode, string Left, string Right, string Pair)
        {
            string sql = "insert into Daily_child_table(Membercode, Total_leftchild, Total_rightchild, Pair,Date, Time,Deleteid, Closingno, Idate) values ('" +
                         MemberCode + "','" + Left + "','" + Right + "','" + Pair + "','" + Date + "','" + Time + "','" + DeleteID + "','" + ClosingNumber +
                         "',N'" + Idate + "')";
            int i = imp.InsertUpdateDelete(sql);
        }

        public void UpdateInToDailyChildTableInDataBase(string MemberCode, string Date, string ClosingNumber, string Left, string Right, string Pair)
        {
            string sql = "update Daily_child_table set Total_leftchild='" + Left + "', Total_rightchild='" + Right + "', Pair='" + Pair + "' where Membercode='" + MemberCode +
                         "' and Date='" + Date + "' and Closingno='" + ClosingNumber + "'";
            int i = imp.InsertUpdateDelete(sql);
        }
        #endregion
